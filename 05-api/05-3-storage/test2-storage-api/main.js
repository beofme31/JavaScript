...
const apiTest = () => {
  ...
  // 데이터 저장 구조는 key-value, key로 식별해서 여러건의 데이터 저장
  sessionStorage.setItem('data1', '홍길동')
  // key를 마치 객체의 변수처럼 사용
  sessionStorage.data2 = '홍길동'
  // 문자열이 아닌 각 타입의 데이터 저장시 에러가 나지 않는다.
  // 데이터 타입 유지 안 된다. 문자열로만 저장된다.
  ...
  console.log('data4', sessionStorage.getItem('data4'), typeof sessionStorage.getItem('data4'));

  // 객체를 storage에 저장하겠다면
  // storage는 문자열로만 저장되므로 코드에서 객체를 문자열로 변형해서 저장
  // 획득시에 다시 객체로 변형해서 사용
