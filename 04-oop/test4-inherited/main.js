...
console.log(new Shape('react1'));

// 후배 개발자 입장에서 선임 개발자가 만들어놓은 Shape를 상속받아서
// 자신이 구체적으로 만들어야 하는 도형을 개발하겠다는 입장
function Rectangle(name, width, height) {
  // name 데이터가 객체에 유지되어야 한다.
  // 객체별로 name 데이터가 다르게 유지 -> this에 등록
  // 아래처러 작성해서 객체별로 데이터가 유지되게 할수도 있지만
  // this.name = name

  // 다른 생성자 함수에 name이 작성되어 있어서 그걸 그대로 이용
...
// Shape 생성자 함수를 같이 실행시킨거다. Shape에 등록한 것이 Rectangle에서
// 자신의 것처럼 사용하고 있다. -> 상속 개념
